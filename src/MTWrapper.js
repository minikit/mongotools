...
class MTWrapper {
    ...
    if (uri != null) {
      if (!uri.includes("/")) {
        return reject({
          ...
          message: "uri: database name for dump is required.",
        });
      }
  ...
  commandDumpCollectionsFromOptions(options, command) {
    ...
    if (isOptionSet(options, "collection") && !isSingleValue(options.collection)) {
      return reject({
        error: "INVALID_OPTIONS",
        message: '"collection" option must be a single value.',
      });
    }
    // deprecated includeCollections
  ...
  mongodump(options) {
    ...
    return new Promise(async function (resolve, reject) {
      if (!("db" in options) && !("uri" in options)) {
        return reject({
          error: "INVALID_OPTIONS",
          message: "db: database name for dump is required.",
        });
      }
      ...
      if (!fs.existsSync(path)) {
        await fsPromise.mkdir(path, { recursive: true }).catch(err => {
          return reject({
            error: "INVALID_OPTIONS",
            message: "path: cannot create " + path + " :" + err,
          });
        });
      }
      ...
      try {
        ...
        if (dump.status === 0) {
          ...
          reject({
            ...
            message: `Binary ${dumpCmd} not found`,
          });
        } else {
          reject({
            error: "COMMAND_ERROR",
  ...
  mongorestore(options, toRestore = null) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (dumpFile === null || dumpFile === undefined) {
        return reject({
          error: "INVALID_OPTIONS",
          message: "dumpFile: mongo dump file is required.",
        });
      }
      ...
      try {
        ...
        if (restore.status === 0) {
          ...
          reject({
            error: "COMMAND_NOT_FOUND",
            message: `Binary ${restoreCmd} not found`,
          });
        } else {
          reject({
            error: "COMMAND_ERROR",
